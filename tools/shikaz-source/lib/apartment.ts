...
import { buildApartment, mergeModel, zones } from "./scene-model";
...
export function roomAt(x:number,z:number){return zones.find(r=>x>=r.x&&x<=r.x+r.w&&z>=r.z&&z<=r.z+r.d)?.id||"hall";}
export function insideApartment(x:number,z:number){return zones.some(r=>x>r.x-.025&&x<r.x+r.w+.025&&z>r.z-.025&&z<r.z+r.d+.025);}
export function createApartment(host:HTMLDivElement,cb:Callbacks):ApartmentController {
 ...
 let dirty=true;
 const scene=new T.Scene();scene.background=new T.Color("#242936");
 ...
 renderer.outputColorSpace=T.SRGBColorSpace;renderer.toneMapping=T.ACESFilmicToneMapping;renderer.toneMappingExposure=.86;
 renderer.shadowMap.enabled=true;renderer.shadowMap.type=T.PCFShadowMap;
 renderer.domElement.setAttribute("aria-label","Shikaz Homes interactive 3D scene");host.appendChild(renderer.domElement);
 const camera=new T.PerspectiveCamera(63,host.clientWidth/host.clientHeight,.055,70);camera.rotation.order="YXZ";
 const orbit=new OrbitControls(camera,renderer.domElement);orbit.enabled=false;orbit.enableDamping=!reduced;orbit.dampingFactor=.075;orbit.minDistance=3;orbit.maxDistance=29;orbit.maxPolarAngle=Math.PI/2-.05;
 ...
 furnishRooms(model);furnishServices(model);
 mergeModel(model);
 let vertices=0,meshes=0;model.root.traverse(o=>{if(o instanceof T.Mesh){meshes++;vertices+=o.geometry.attributes.position.count}});host.dataset.sceneMeshes=String(meshes);host.dataset.sceneVertices=String(vertices);
 const pmrem=new T.PMREMGenerator(renderer);const envScene=new RoomEnvironment();const envTarget=pmrem.fromScene(envScene,.04);scene.environment=envTarget.texture;scene.environmentIntensity=.32;envScene.dispose();pmrem.dispose();
 const ambient=new T.AmbientLight("#fff8eb",.28);scene.add(ambient);
 const sky=new T.HemisphereLight("#eff5ff","#a7a299",.68);scene.add(sky);
 const sun=new T.DirectionalLight("#fff4db",1.25);sun.position.set(-3,8,-4);sun.target.position.set(0,0,4);sun.castShadow=true;
 sun.shadow.mapSize.set(mobile?1024:2048,mobile?1024:2048);sun.shadow.camera.left=-10;sun.shadow.camera.right=10;sun.shadow.camera.top=12;sun.shadow.camera.bottom=-12;sun.shadow.camera.near=.1;sun.shadow.camera.far=24;sun.shadow.bias=-.0003;sun.shadow.normalBias=.025;sun.shadow.radius=3;scene.add(sun,sun.target);
 renderer.shadowMap.autoUpdate=false;renderer.shadowMap.needsUpdate=true;
 const lamps:T.PointLight[]=[];
 for(const [x,y,z,power] of [[2.1,2.55,2.7,12],[2.8,2.5,7.1,8],[-3.1,2.5,1.8,8],[-2.4,2.5,8.4,11],[-.8,2.5,4.8,5],[-2.7,2.4,4.25,3],[-2.7,2.4,5.65,3]]) {
   const light=new T.PointLight("#fff1d6",power,8,2);light.position.set(x,y,z);scene.add(light);lamps.push(light);
 }
 ...
 const ao=new GTAOPass(scene,camera,host.clientWidth,host.clientHeight);ao.blendIntensity=.85;ao.updateGtaoMaterial({radius:.22,distanceExponent:1.4,thickness:.25,scale:1});composer.addPass(ao);composer.addPass(new OutputPass());
 ao.enabled=!mobile;
 ...
 function setWalkPose(id:string) {
   dirty=true;
   const view=roomViews[id] || roomViews.living;camera.position.set(...view.position);
 ...
 function setMode(next:ViewMode) {
   ...
   else {orbit.target.set(-.3,0,5.15);orbit.enableRotate=next==="overview";orbit.minPolarAngle=next==="plan"?0:.15;orbit.maxPolarAngle=next==="plan"?.02:Math.PI/2-.05;const eye:[number,number,number]=next==="plan"?[-.3,20,5.16]:[12,13,18];camera.position.set(...eye);camera.lookAt(orbit.target);orbit.update()}
 }
 ...
 function render(time:number) {
   ...
   if(loaded&&!announced){announced=true;host.dataset.ready="true";cb.onReady()}
 }
 frame=requestAnimationFrame(render);
 return {
   goRoom(id){setMode("walk");setWalkPose(id);host.focus({preventScroll:true})},
   setMode,
   setNight(night){dirty=true;ambient.intensity=night?.20:.28;sky.intensity=night?.22:.68;sun.intensity=night?.08:1.25;lamps.forEach((l,i)=>{l.color.set(night?"#ffd590":"#fff1d6");l.intensity=lampPowers[i]*(night?1.3:1)});renderer.toneMappingExposure=night?.90:.86},
   move(x,z){padX=x;padZ=z},
   dispose(){disposed=true;cancelAnimationFrame(frame);observer.disconnect();window.removeEventListener("keydown",keydown);window.removeEventListener("keyup",keyup);window.removeEventListener("blur",blur);renderer.domElement.removeEventListener("pointerdown",down);renderer.domElement.removeEventListener("pointermove",move);renderer.domElement.removeEventListener("pointerup",up);renderer.domElement.removeEventListener("pointercancel",up);orbit.dispose();ao.dispose();composer.dispose();envTarget.dispose();const materials=new Set<T.Material>(),textures=new Set<T.Texture>();scene.traverse(o=>{if(o instanceof T.Mesh){o.geometry.dispose();for(const mat of Array.isArray(o.material)?o.material:[o.material])materials.add(mat)}});for(const mat of materials){for(const value of Object.values(mat))if(value instanceof T.Texture)textures.add(value);mat.dispose()}for(const t of textures)t.dispose();renderer.dispose();renderer.domElement.remove()}
 };
}
