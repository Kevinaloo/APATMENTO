...
(async()=>{
 ...
 let html=await fs.readFile(path.join(out,'index.html'),'utf8');html=html.replace(/\.\/assets\/viewer-[^"]+\.js/,'./assets/'+path.basename(entry));await fs.writeFile(path.join(out,'index.html'),html);
