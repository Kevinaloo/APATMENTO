...
Hosts open **Partner Hub → My Listings → Offers & campaigns**, or **Offers** on a stay card. Admins use **Offers & campaigns** in the existing admin navigation. These are embedded sections, using the existing fonts, colours and layout.

## Host controls

Create a draft, choose a discount and the minimum nightly amount you are comfortable receiving, and preview the protected price. Activate immediately or schedule the booking window. Optional rules cover minimum/maximum nights, advance booking, eligible weekdays and excluded nights. Every occupied night must qualify; the checkout date is not an occupied night. Pause or end an offer at any time for future bookings.
...
- Booking windows use explicit timestamps (entered as UTC). The offer's named time zone controls advance-booking days. Stay windows use calendar dates, including latest checkout.
