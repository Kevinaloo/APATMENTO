#!/usr/bin/env bash
# ──────────────────────────────────────────────────────────────────────────
# Stays facade gate UI test runner.
#
...
for _ in $(seq 1 40); do
  curl -sf "http://localhost:$PORT/apartments.html" >/dev/null 2>&1 && break
  sleep 0.25
done

node tests/ui/stay-gate.test.js
