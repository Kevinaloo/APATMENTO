#!/usr/bin/env bash
# ──────────────────────────────────────────────────────────────────────────
# Dashboard service rails UI test runner.
#
# Starts the shared stub server on a free port, runs the browser suite
# against it, and tears everything down. Playwright is installed into a
# scratch dir rather than the repo, for the same reason as the ambassador
# suite: this is a static site and a package.json would change how it
# deploys.
#
#   ./tests/ui/run-dashboard-rails.sh
# ──────────────────────────────────────────────────────────────────────────
set -euo pipefail
cd "$(dirname "$0")/../.."
...
for _ in $(seq 1 40); do
  curl -sf "http://localhost:$PORT/dashboard.html" >/dev/null 2>&1 && break
  sleep 0.25
done

node tests/ui/dashboard-rails.test.js
